...
class auth{
    ...
    public function signup(){
        global $conf, $ObjFncs, $lang, $Objsendmail, $Objdb;
        // code for signup
        if(isset($_POST['signup'])){
            ...
            $fullname = $_SESSION['fullname'] = $Objdb->escape_values(ucwords(strtolower($_POST['fullname'])));
            $email = $_SESSION['email'] = $Objdb->escape_values(strtolower($_POST['email']));
            $password = $_SESSION['password'] = $Objdb->escape_values($_POST['password']);
            ...
            // Check if email already exists in the database
     		$spot_email_res = $Objdb->count_results(sprintf("SELECT email FROM users WHERE email = '%s' LIMIT 1", $email));
			if ($spot_email_res > 0){
            ...
            if (!count($errors)) {
                ...
                $user_data = array('fullname'=>$fullname, 'email'=>$email, 'password'=>$hashed_password, 'status'=>'Pending', 'verify_code'=>$verification_code, 'code_expiry_time' => $code_expiry_time);

                // Insert user data into the database
                $save_user = $Objdb->insert('users', $user_data);
                ...
                exit();
                    ...
                    error_log('Signup Error: ' . $save_user);
...
public function verify_code(){
    ...
    if(isset($_POST['verify_code'])){
        ...
        // Retrieve and sanitize user inputs
        $verification_code = $Objdb->escape_values($_POST['verification_code']);
        ...
        if (!count($errors)) {
            ...
            if ($Objdb->count_results($spot_user_query) > 0){
                $check_userId_res = $Objdb->select($spot_user_query);
                ...
                $update_user = $Objdb->update('users', $update_data, sprintf("verify_code = '%s'", $verification_code));
                ...
                if($update_user === TRUE){
                   ...
                   unset($_SESSION['email']);
                    ...
                    error_log('Verification Error: ' . $update_user);
...
public function forgot_password(){
    ...
    if(isset($_POST['send_code'])){
        ...
        // Retrieve and sanitize user inputs
        $email = $_SESSION['email'] = $Objdb->escape_values(strtolower($_POST['email']));
        $origin = $_SESSION['origin'] = $Objdb->escape_values($_POST['origin']);
        // Set validation rules
        if (empty($email)) {
            $errors['mail_error'] = "Email is required";
        }
        ...
        $spot_email_res = $Objdb->count_results(sprintf("SELECT email, fullname FROM users WHERE email = '%s' LIMIT 1", $email));
        if ($spot_email_res == 0){
        ...
        if (!count($errors)) {
            ...
            $update_data = array('verify_code'=>$verification_code, 'code_expiry_time' => $code_expiry_time);
            $update_user = $Objdb->update('users', $update_data, sprintf("email = '%s'", $email));

            if($update_user === TRUE){
                // Fetch user's fullname for email personalization
                $user_info = $Objdb->select(sprintf("SELECT fullname FROM users WHERE email = '%s' LIMIT 1", $email));
...
public function change_password(){
   ...
   if(isset($_POST['change_password'])){
        ...
        $errors = [];
        // Retrieve and sanitize user inputs
        $new_password = $_SESSION['new_password'] = $Objdb->escape_values($_POST['new_password']);
        $confirm_password = $_SESSION['confirm_password'] = $Objdb->escape_values($_POST['confirm_password']);
        ...
        if (!count($errors)) {
            ...
            $user_info = $Objdb->select(sprintf("SELECT password, email, fullname FROM users WHERE userId = '%d' LIMIT 1", $_SESSION['change_userId_pass']));
            if ($user_info) {
                ...
                $update_user = $Objdb->update('users', $update_data, sprintf("userId = '%d'", $_SESSION['change_userId_pass']));
                if($update_user === TRUE){
                    // Prepare variables to replace in email template
                    $variables = [
                        'site_name' => $conf['site_name'],
                    ...
                    unset($_SESSION['email']);
                    unset($_SESSION['origin']);
                    unset($_SESSION['change_userId_pass']);
...
public function signin(){
    ...
    if(isset($_POST['signin'])){
        ...
        if (!count($errors)) {
            ...
            $user_info = $Objdb->select(sprintf("SELECT userId, fullname, email, password, status FROM users WHERE email = '%s' LIMIT 1", $email));
            if ($user_info) {
